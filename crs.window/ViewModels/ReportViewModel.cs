...
namespace crs.window.ViewModels
{
    public class ReportViewModel : Crs_BindableBase
    {
        ...
        async void ExecuteViewCommand(ReportItem parameter)
        {
            ...
            var viewName = selectedItem.Item1 switch
            {
                ReportType.评估报告 => Crs_Dialog.EvaluateReport,
                ReportType.训练报告 => Crs_Dialog.TrainReport,
            ...
            await Crs_DialogEx.Show(viewName, Crs_DialogToken.TopContent)
                ...
                .Initialize<IDialogCommon<int?, int?, int?>>(vm => vm.Execute(patientId, moduleId, resultId))
