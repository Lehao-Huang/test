...
larger than this value. Used only when --enable-endpoint is true.")]
        ...
        public readonly int AwakenTime = 6;
        public readonly int DelayTime = 3;
...
";
        ...
        private void Run(Options options, TaskCompletionSource<(bool, string)> tcls, CancellationToken token)
        {
            try
            {
                ...
                Debug.WriteLine(PortAudio.VersionInfo.versionText);
                Console.WriteLine(PortAudio.VersionInfo.versionText);

                PortAudio.Initialize();

                Debug.WriteLine($"Number of devices: {PortAudio.DeviceCount}");
                for (int i = 0; i != PortAudio.DeviceCount; ++i)
                {
                    Debug.WriteLine($" Device {i}");
                    ...
                    Debug.WriteLine($"   Max input channels: {deviceInfo.maxInputChannels}");
                    Console.WriteLine($"   Max input channels: {deviceInfo.maxInputChannels}");

                    Debug.WriteLine($"   Default sample rate: {deviceInfo.defaultSampleRate}");
                    Console.WriteLine($"   Default sample rate: {deviceInfo.defaultSampleRate}");
                }
                int deviceIndex = PortAudio.DefaultInputDevice;
                if (deviceIndex == PortAudio.NoDevice)
                {
                    Debug.WriteLine("No default input device found");
                ...
                Debug.WriteLine($"Use default device {deviceIndex} ({info.name})");
                ...
                Debug.WriteLine("Started! Please speak");
                ...
                Debug.WriteLine("PortAudio.Terminate");
                Console.WriteLine("PortAudio.Terminate");
            }
            catch (Exception ex)
            {
